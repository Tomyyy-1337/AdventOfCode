...
fn main() {
    ...
    let mut grid = Grid::from_string(&std::fs::read_to_string(path).unwrap());

    grid.next_beam_step(0, 0, Direction::Right);

    let sum = grid.energy.iter().filter(|x| **x == Status::Energized).count();
