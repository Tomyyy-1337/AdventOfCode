...
impl Board {
    ...
    fn find_best_path(&mut self) -> i32 {
        let mut queue: PriorityQueue<(i32, i32, i32, Direction, i32), i32> = PriorityQueue::new();
        queue.push((0, 0, 0, Direction::Right, 0), 0);
        queue.push((0, 0, 0, Direction::Down, 0), 0);

        while let Some(((x, y, current_loss, direction, dir_count), _)) = queue.pop() {
            if x == self.width as i32 - 1 && y == self.height as i32 - 1 && dir_count >= 4 {
                return current_loss;
            }
            ...
            let dirs: Vec<Direction> = match dir_count {
                0..=3 => vec![direction],
                4..=9 => direction.not_backwards(),
...
fn main() {
    ...
    let loss = Board::from_str(&std::fs::read_to_string(path).unwrap()).find_best_path();
