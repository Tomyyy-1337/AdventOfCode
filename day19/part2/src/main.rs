...
impl Operator {
    ...
    fn evaluate_range(&self, range: &Range) -> Option<(Tag, Range, Range)> {
        match self {
            Operator::Const(tag) => Some((tag.clone(), range.clone(), Range::new(0,0))),
...
impl Range { 
    fn new(lower: i32, upper: i32) -> Range {
        Range { ranges: vec![(lower, upper), (lower, upper), (lower, upper), (lower, upper)] }
    }
...
fn main() {
    ...
    let mut count = 0;
    let mut stack = vec![(Tag::Named("in".to_string()), Range::new(1,4001))];
    while let Some((tag, mut range)) = stack.pop() {
        for operator in workflow_map.get(&tag).unwrap().filter.iter() {
            if let Some((tag, range_hit, range_miss)) = operator.evaluate_range(&range) {
                match tag {
                    Tag::Named(_) => stack.push((tag, range_hit)),
                    Tag::Accept => count += range_hit.abs(),
                    Tag::Reject => (),
                }
                range = range_miss;
            } 
